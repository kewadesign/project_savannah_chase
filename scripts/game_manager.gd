...
const LION_SCENE_PATH = "res://scenes/lion.tscn"
const GIRAFFE_SCENE_PATH = "res://scenes/giraffe.tscn"
const GAZELLE_SCENE_PATH = "res://scenes/gazelle.tscn"

# Materialien für die Figuren
const WHITE_MATERIAL_PATH = "res://assets/materials/figure_white.tres"
const BLACK_MATERIAL_PATH = "res://assets/materials/figure_black.tres"
const HIGHLIGHT_MATERIAL_PATH = "res://assets/materials/highlight_material.tres"
...
func _ready():
	...
	print("DEBUG: Pfade zu den Figuren und Materialien:")
	print("LION_SCENE_PATH: " + LION_SCENE_PATH)
	print("GIRAFFE_SCENE_PATH: " + GIRAFFE_SCENE_PATH)
	print("GAZELLE_SCENE_PATH: " + GAZELLE_SCENE_PATH)
	print("WHITE_MATERIAL_PATH: " + WHITE_MATERIAL_PATH)
	print("BLACK_MATERIAL_PATH: " + BLACK_MATERIAL_PATH)
	print("HIGHLIGHT_MATERIAL_PATH: " + HIGHLIGHT_MATERIAL_PATH)
...
func load_materials():
	# Versuche die Materialien zu laden
	white_material = load(WHITE_MATERIAL_PATH)
	black_material = load(BLACK_MATERIAL_PATH)
	highlight_material = load(HIGHLIGHT_MATERIAL_PATH)
	
	# Fehlerbehandlung für weißes Material
	if white_material == null:
		print("FEHLER: Weißes Material konnte nicht geladen werden: " + WHITE_MATERIAL_PATH)
		
		# Alternative Pfade versuchen
		var alt_path = "res://figure_white.tres"
		print("Versuche alternativen Pfad für weißes Material: " + alt_path)
		white_material = load(alt_path)
		
		if white_material == null:
			print("FEHLER: Auch der alternative Pfad für weißes Material konnte nicht geladen werden!")
	
	# Fehlerbehandlung für schwarzes Material
	if black_material == null:
		print("FEHLER: Schwarzes Material konnte nicht geladen werden: " + BLACK_MATERIAL_PATH)
		
		# Alternative Pfade versuchen
		var alt_path = "res://figure_black.tres"
		print("Versuche alternativen Pfad für schwarzes Material: " + alt_path)
		black_material = load(alt_path)
		
		if black_material == null:
			print("FEHLER: Auch der alternative Pfad für schwarzes Material konnte nicht geladen werden!")
	
	# Fehlerbehandlung für Highlight-Material
	if highlight_material == null:
		print("FEHLER: Highlight-Material konnte nicht geladen werden: " + HIGHLIGHT_MATERIAL_PATH)
		
		# Alternative Pfade versuchen
		var alt_path = "res://highlight_material.tres"
		print("Versuche alternativen Pfad für Highlight-Material: " + alt_path)
		highlight_material = load(alt_path)
		
		if highlight_material == null:
			print("FEHLER: Auch der alternative Pfad für Highlight-Material konnte nicht geladen werden!")
...
func place_piece(piece_scene_path: String, is_white: bool, x: int, y: int):
	...
	# Wir nehmen an, dass es einen "Pieces" Node gibt, dem wir die Figuren hinzufügen
	var pieces_node = get_node("../Pieces")
	if pieces_node:
		pieces_node.add_child(piece_instance)
	else:
		# Fallback: Wenn kein Pieces-Node gefunden wird, zur Szene hinzufügen
	...
	# Die korrekte Position basierend auf den GridMap-Einstellungen
